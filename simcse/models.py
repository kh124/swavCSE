...
def cl_init(cls, config):
    ...
    cls.pooler_q_type = cls.model_args.pooler_q_type
    cls.pooler = Pooler(cls.model_args.pooler_type)
    cls.pooler_q = Pooler(cls.model_args.pooler_q_type)
    if cls.model_args.pooler_type == "cls":
        cls.mlp = MLPLayer(config)
    if cls.model_args.pooler_q_type == "cls":
        cls.mlp_q = MLPLayer(config)
    cls.sim = Similarity(temp=cls.model_args.temp)
    cls.init_weights()
    config.attention_probs_dropout_prob = 0.0
    config.hidden_dropout_prob = 0.0
    cls.encoder_q = BertModel(config, add_pooling_layer=False)
    config.attention_probs_dropout_prob = 0.1
    config.hidden_dropout_prob = 0.1

    # print("here is the initial bert:\n", cls.bert)
    # print("here is the encoder_q:\n", cls.encoder_q)
...
@torch.no_grad()
def dequeue_and_enqueue(cls, keys, keys_2):
    ...
    cls.queue_2[ptr:ptr + batch_size, :] = keys_2
    # cls.queue[ptr:ptr + batch_size, :] = keys
...
@torch.no_grad()
def sinkhorn(cls, out):
    Q = torch.exp(out / cls.model_args.epsilon).t() # Q is K-by-B for consistency with notations from our paper
    B = Q.shape[1]
    ...
    for it in range(cls.model_args.sinkhorn_iterations):
...
def cl_forward(cls,
    ...
    if cls.model_args.do_stronger:
        # set shuff
        cls.encoder_q.set_flag("data_aug_shuffle", True)
        # set cutoff
        # cls.encoder_q.set_flag("data_aug_cutoff", True)
        # cls.encoder_q.set_flag("data_aug_cutoff.direction", "random")
        # cls.encoder_q.set_flag("data_aug_cutoff.rate", cls.model_args.cutoff_rate)
        # set others here
        ...
        with torch.no_grad():
            ...
            # with torch.no_grad():
            stronger_outputs = cls.encoder_q(
                stronger_input_ids,
                ...
                output_hidden_states=True if cls.model_args.pooler_q_type in ['avg_top2', 'avg_first_last'] else False,
                return_dict=True,
            )

            stronger_pooler_output = cls.pooler_q(stronger_attention_mask, stronger_outputs)

            if cls.pooler_q_type == "cls":
                stronger_pooler_output = cls.mlp_q(stronger_pooler_output)
    ...
    if cls.model_args.do_clustering:
        # normalize the prototypes
        with torch.no_grad():
            w = cls.prototypes.weight.data.clone()
            w = nn.functional.normalize(w, dim=1)
            cls.prototypes.weight.copy_(w)

        # maping to unit sphere
        x = nn.functional.normalize(z1, dim=1)
        x_2 = nn.functional.normalize(z2, dim=1)
        # queue = nn.functional.normalize(cls.queue.clone().detach(),dim=1)

        scores = cls.prototypes(x)
        scores_2 = cls.prototypes(x_2)

        #compute Q here
        with torch.no_grad():
            s = scores.detach()
            s = torch.cat((torch.mm(
                cls.queue.clone().detach(),
                cls.prototypes.weight.t()
            ), s))

            s_2 =scores_2.detach()
            s_2 = torch.cat((torch.mm(
                cls.queue_2.clone().detach(),
                cls.prototypes.weight.t()
            ), s_2))

            # get assignments
            q = sinkhorn(cls, s)[-batch_size:]
            q_2 = sinkhorn(cls, s_2)[-batch_size:]

            dequeue_and_enqueue(cls, x, x_2)

        # compute clustering_loss here
        cluster_loss = 0
        scores = scores / cls.model_args.temp
        scores_2 = scores_2 / cls.model_args.temp
        # subloss -= torch.mean(torch.sum(q * F.log_softmax(x, dim=1), dim=1))
        cluster_loss -= torch.mean(torch.sum(q * F.log_softmax(scores_2, dim=1), dim=1))
        cluster_loss -= torch.mean(torch.sum(q_2 * F.log_softmax(scores, dim=1), dim=1))
        cluster_loss /= 2


        # z2_bank_cos = cls.sim(z2.unsqueeze(1), cls.queue.clone().detach().unsqueeze(0))
    ...
    if cls.model_args.do_stronger or cls.model_args.do_mlm:
        ...
        z3_bank_cos = cls.sim(z3.unsqueeze(1), cls.queue.clone().detach().unsqueeze(0))

        softmax = nn.Softmax()
        z2_bank_cos_softmax = softmax(z2_bank_cos, dim=1)
        z3_bank_cos_softmax = softmax(z3_bank_cos, dim=1)
    ...
    if cls.model_args.do_stronger or cls.model_args.do_mlm or cls.model_args.do_clustering:
        # loss = loss + cls.model_args.mlm_weight * ld_loss
        loss = loss + cls.model_args.mlm_weight * cluster_loss
        # loss = cluster_loss
...
class BertForCL(BertPreTrainedModel):
    ...
    def __init__(self, config, *model_args, **model_kargs):
        ...
        self.register_buffer("queue_2", torch.randn(self.model_args.bank_size, self.model_args.hidden_len))

        self.queue = nn.functional.normalize(self.queue, dim=1)  # across queue instead of each example
        self.queue_2 = nn.functional.normalize(self.queue_2, dim=1)  # across queue instead of each example

        self.register_buffer("queue_ptr", torch.zeros(1, dtype=torch.long))

        self.prototypes =nn.Linear(self.model_args.hidden_len, self.model_args.nmb_prototypes, bias=False)
